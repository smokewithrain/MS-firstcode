...
class PSO_inverse_design:
    ...
    def run(self):
        ...
        for iter in range(self.max_iterations):                   # 每轮迭代
            ...
            if unimprove_count >= 10:     
                if self.plot:                                                                          # 当连续未改进时，对适应度最差的一半粒子随机扰动
                    print("连续未改进, 随机扰动")
                sorted_idx = np.argsort(-cur_fitness)                            # 默认升序，这里降序排列
                worse_half_idx = sorted_idx[:self.num_particles // 5]
                # 随机扰动
                rand_move = np.random.rand(len(worse_half_idx), self.particle_dim) * 0.8 - 0.4                       # [-0.2, 0.2]                  
    ...
    def plot_fitness_progress(self):
        ...
        plt.savefig(output_dir / f'{self.title}/fitness_progress.png', dpi=700, bbox_inches='tight')
        plt.close()

        pd.DataFrame(self.fitness_progress).to_csv(output_dir / f"{self.title}/fitness.csv", header=False, index=False)
    ...
    def plot_optimization_result(self):
        ...
        plt.savefig(output_dir / f'{self.title}/A_curve_comparison.png', dpi=700, bbox_inches='tight')
        plt.savefig(output_dir / f'{self.title}/A_curve_comparison.pdf', dpi=700, bbox_inches='tight')
        plt.close()
    ...
    def save_results(self):
        ...
        best_params.to_csv(output_dir / f'{self.title}/best_parameters.csv', index=False, header=False)
        
        # 保存预测曲线
        S11_design = self.forward_predict(self.gbest)
        S11_design = pd.DataFrame(S11_design.reshape(1, -1))
        S11_design.to_csv(output_dir / f"{self.title}/S11_design.csv", index=False, header=False)
